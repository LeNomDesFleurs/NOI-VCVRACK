...
def components_to_source(components, slug):
	...
	if len(components['params']) > 0:
		for c in components['params']:
			source += "\n"
			source += f"auto {c['name']}_PARAMpos = Vec({c['cx']}, {c['cy']});"
	# Inputs
	source += "\n"
	if len(components['inputs']) > 0:
		for c in components['inputs']:
			source += "\n"
			source += f"auto {c['name']}_INPUTpos =Vec({c['cx']}, {c['cy']});"
	# Outputs
	source += "\n"
	if len(components['outputs']) > 0:
		for c in components['outputs']:
			source += "\n"
			source += f"auto {c['name']}_OUTPUTpos = Vec({c['cx']}, {c['cy']});"
	# Lights
	source += "\n"
	if len(components['lights']) > 0:
		for c in components['lights']:
			source += "\n"
			source += f"auto {c['name']}_LIGHTpos = Vec({c['cx']}, {c['cy']});"
	...
	if len(components['params']) > 0:
		for c in components['params']:
			source += "\n"
			source += f"addParam(createParamCentered<RoundBlackKnob>(mm2px({c['name']}_PARAMpos), module, {slug}::{c['name']}_PARAM));"
	# Inputs
	source += "\n"
	if len(components['inputs']) > 0:
		for c in components['inputs']:
			source += "\n"
			source += f"addInput(createInputCentered<PJ301MPort>(mm2px({c['name']}_INPUTpos), module, {slug}::{c['name']}_INPUT));"
	...
	if len(components['outputs']) > 0:
		for c in components['outputs']:
			source += "\n"
			source += f"addOutput(createOutputCentered<PJ301MPort>(mm2px({c['name']}_OUTPUTpos), module, {slug}::{c['name']}_OUTPUT));"
	# Lights
	source += "\n"
	if len(components['lights']) > 0:
		for c in components['lights']:
			source += "\n"
			source += f"addChild(createLightCentered<SmallLight<WhiteLight>>(mm2px({c['name']}_LIGHTpos), module, {slug}::{c['name']}_LIGHT));"
